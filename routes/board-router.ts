 ...
 */
boardRouter.post("/add", authUserMiddleWare, async function (req, res, next) {
    try {
 ...
 * Nécessite d'être connecté : OUI
 */
boardRouter.get("/", authUserMiddleWare, async function (req, res, next) {
    try {
        if (await isDevConnected(req)) {
            const connection = await DatabaseUtils.getConnection();
            const boardService = new BoardServiceImpl(connection);
 ...
 * Nécessite d'être connecté : OUI
 */
boardRouter.get("/:id", authUserMiddleWare, async function (req, res, next) {
    try {
        if (await isDevConnected(req)) {
            const connection = await DatabaseUtils.getConnection();
            const boardService = new BoardServiceImpl(connection);
 ...
 * Nécessite d'être connecté : OUI
 */
boardRouter.put("/:id", authUserMiddleWare, async function (req, res, next) {
    try {
        if (await isDevConnected(req)) {
            const id = req.params.id;
            const name = req.body.name;
 ...
 * Nécessite d'être connecté : OUI
 */
boardRouter.delete("/:id", authUserMiddleWare, async function (req, res, next) {
    try {
        if (await isDevConnected(req)) {
            const connection = await DatabaseUtils.getConnection();
            const boardService = new BoardServiceImpl(connection);
